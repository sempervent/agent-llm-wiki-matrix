...
7. **Campaign (multi-run):** Optional **`alwm benchmark campaign run`** loads a **campaign definition** and invokes the harness once per expanded cell (suite × provider × scoring × browser axes), writing a **campaign manifest** and **campaign summary** at the campaign root and standard **`benchmark_manifest`** trees under **`runs/runNNNN/`**. Longitudinal reporting consumes those member manifests the same way as standalone runs; matrices and reports remain per member run unless you aggregate downstream.
