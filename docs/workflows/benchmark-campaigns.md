...
**Walkthrough (committed examples):** [campaign-walkthrough.md](campaign-walkthrough.md) · **End-to-end publication:** [campaign-result-pack-publication.md](campaign-result-pack-publication.md) · **Verification:** [verification.md](verification.md) · **MkDocs site:** [docs-site.md](docs-site.md) · **Wiki:** [Campaign orchestration (index)](../wiki/campaign-orchestration.md) · **ADR:** [Benchmark campaign orchestration](../architecture/adr/0001-benchmark-campaign-orchestration.md) · **Tracking:** [Campaign orchestration](../tracking/campaign-orchestration.md).
...
| ``campaign-summary.md`` | Publication-style index: **metadata**, **experiment fingerprints**, **execution context**, **Snapshot digest** (short headline copy; full tables in comparative + semantic reports), **Member run index**, optional **Generated reports** links. Digest: **Judge & semantic signals** (one ``###`` with ``####`` confidence + axis hotspots when semantic summary exists), sweep spreads, backends, instability, mode gaps, **FT-\***. |
...
| ``reports/campaign-report.md`` | **Comparative report:** **Executive summary**, optional **Judge variance (abbreviated)** embed (pointer to ``campaign-semantic-summary.md``), ``---`` then sweep dimensions, member mean-score tables, fingerprint-axis interpretation, backends, instability, mode gaps, **FT-\*** tags, failure atlas. **browser_mock** runs add compact **Browser evidence** intro + tables. |
...
A **campaign result pack** is the **canonical outward-facing bundle** for a completed campaign: a **git-friendly** directory that copies the important parts of a finished campaign tree—**campaign manifest**, **summaries**, **semantic summary** (when present), **comparative report** + **analysis JSON** (when present), optional **`campaign-dry-run.json`** (plan-only campaigns), and **selected member runs** (full trees by default). The pack root adds **`campaign-result-pack.json`** (kind **`campaign_result_pack`**) and **`INDEX.md`**. The JSON records optional layer paths (for example **`campaign_dry_run_json`**) plus derived **`membership_scope`** (**`all_runs`** / **`subset`** / **`unknown`**) and **`optional_layers_present`** (semantic / comparative / analysis / dry-run). **`INDEX.md`** targets **cold reviewers**: **For reviewers (read this first)** (what the bundle is, reading order), **Scope of this bundle**, **What is included**, **Publish-ready checklist**, provenance, fingerprints, and validation commands—without repeating the old wide **Bundle completeness** grid.
...
| Optional layers | When semantic files exist: `alwm validate …/campaign-semantic-summary.json campaign_semantic_summary`; confirm **`optional_layers_present`** / **What is included** in **`INDEX.md`** matches expectations |
...
**Comparing two packs (CLI):** emit **`pack-compare.json`** (kind **`campaign_result_pack_comparison`**) and **`pack-compare-report.md`**. Report order: **`## At a glance`** (non-causal summary from JSON **`reader_interpretation`** — no duplicate instability/browser/FT prose), **`## Member run overlap`**, identity and artifact tables, then **`## Analysis deltas`** (pooled means; instability collapsed when all zeros; **FT-\*** split into *With movement* vs *No movement*; compact **browser evidence** subsection; semantic Δ), then portability. Optional **`reader_interpretation`** in JSON keeps full narrative strings for tooling. Use **`--repo-root .`** (from the repo root) for git-friendly relative paths.
...
**Comparing two completed campaign directories (CLI):** emit **`campaign-compare.json`** (kind **`campaign_compare`**) and **`campaign-compare-report.md`** without building result packs first. Markdown order: **`## At a glance`**, **`## Member runs & manifest health`** (overlap + dry_run + manifest run counts), sweep dimensions, fingerprint insights, standard artifact paths, then **`## Analysis deltas`** (same subsection pattern as pack compare). JSON still includes top-level **`browser_evidence`** (rollups) and **`comparative_analysis.browser_evidence_comparison`**. Default is **offline**. Use a fixed **`--created-at`** for reproducible committed examples.
