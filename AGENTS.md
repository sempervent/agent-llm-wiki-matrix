...
- CI parity: `just ci` (ruff, mypy, pytest; excludes `tests/integration/` live provider tests).
- CLI entrypoint: `alwm` (`alwm version`, `alwm info`, `alwm validate …`, `alwm ingest|evaluate|compare|report`, `alwm prompts check|list|show`, `alwm benchmark run` with inline or registry-backed prompts, `alwm providers show`, `alwm browser …` for mock/file browser evidence).
